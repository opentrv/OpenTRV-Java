...
public class TwitterTest
    ...
    public void testSingleTwitterChannel() throws Exception
        ...
        try(final SingleTwitterChannel stc = new SingleTwitterChannelTemperature(config))
            ...
            stc.processStatsMessage(new StatsMessageWithMetadata("{\"@\":\"9999\",\"T|C16\":238,\"L\":3,\"B|cV\":256}", System.currentTimeMillis(), rnd.nextBoolean()));
            assertEquals("message with wrong ID should be ignored", 0, stc.getMessageCount());
            stc.processStatsMessage(new StatsMessageWithMetadata("{\"@\":\""+hexID+"\",\"B|mV\":3230,\"T|C16\":420}", System.currentTimeMillis(), rnd.nextBoolean()));
            assertEquals("message with correct ID should be processed", 1, stc.getMessageCount());
            stc.processStatsMessage(new StatsMessageWithMetadata("{\"@\":\"99999\",\"T|C16\":238,\"L\":3,\"B|cV\":256}", System.currentTimeMillis(), rnd.nextBoolean()));
            assertEquals("message with wrong ID should be ignored", 1, stc.getMessageCount());
            }
        }

    /**Test rendering of temperature without tweeting. */
    @Test
    public void testSingleTwitterChannelTemperature() throws Exception
        {
        final String hexID = "b39a";
        final SingleTwitterChannelConfig config = new SingleTwitterChannelConfig(hexID);
        try(final SingleTwitterChannelTemperature stc = new SingleTwitterChannelTemperature(config))
            {
            assertEquals("26\u00b0C", stc.getTweetBody(new StatsMessageWithMetadata("{\"@\":\""+hexID+"\",\"B|mV\":3230,\"T|C16\":420}", System.currentTimeMillis(), rnd.nextBoolean())));
            assertEquals("19\u00b0C", stc.getTweetBody(new StatsMessageWithMetadata("{\"@\":\""+hexID+"\",\"T|C16\":318}", System.currentTimeMillis(), rnd.nextBoolean())));
            assertEquals("19\u00b0C", stc.getTweetBody(new StatsMessageWithMetadata("@"+hexID.toUpperCase()+";T19C6;L1;O1", System.currentTimeMillis(), rnd.nextBoolean())));
            assertEquals("19\u00b0C", stc.getTweetBody(new StatsMessageWithMetadata("@"+hexID.toUpperCase()+";T19CF;L1;O1", System.currentTimeMillis(), rnd.nextBoolean())));
            }
        }
...
//            }
//        // use the API!
//        jtwit.setStatus("Testing auth... " + rnd.nextInt());
//        }
